...
def prepare_tradcom_classification(training=True, stack=True, sequence_length=5000, features_list=[1,2,3,4], output_dim=3, file_list=None, mean=None, std=None):
    ...
    for filename in file_list:
        ...
        ydf_loc = pd.read_csv(signalfile, names = [0,'signal',], )
...
if action == 'tradcom_simple':
    ...
    training_count = 1 # Does not work with other numbers - the treatment of X and y in prepare_tradcom_classification needs to be changed
    testing_count = 2
    ...
    model = ufcnn_model_concat(day_data_length=50000, regression = False, output_dim=3, features=len(features_list), 
